...
import { Mic, Paperclip, Send, RefreshCw } from "lucide-react";
...
export function Omnibox({ onSubmit, isLoading, isActive, error, onRetry }: OmniboxProps) {
  ...
  return (
    ...
    >
      {!isOnline && (
        <div
          className="absolute -top-12 left-0 right-0 bg-destructive/10 border border-destructive rounded-lg px-4 py-2 text-sm text-destructive"
          role="alert"
        >
      ...
      {error && (
        <div
          className="mb-2 flex items-center gap-2 bg-destructive/10 border border-destructive rounded-lg px-4 py-2 text-sm text-destructive"
          role="alert"
        >
      ...
      <div className="relative flex items-center gap-1 md:gap-2 bg-white dark:bg-slate-900 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 px-4 py-3">
        <Input
          ref={inputRef}
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={isOnline ? "Zadejte lékařský dotaz..." : "Offline - připojte se k internetu"}
